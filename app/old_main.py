...
while True:
    ...
    try:
        conn = psycopg.connect(host='localhost', dbname='posts_fastapi_db', user='postgres', password='Dybala10!', row_factory=dict_row)
...
@app.get("/posts")
def get_posts():
    cursor.execute(""" SELECT * FROM posts """, binary=True)
